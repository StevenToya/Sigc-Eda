...
{
		$enviar_correo = '';
		$structure = "documentos/ambiental/archivos";
		...
		{	
			...
			if($_FILES[$tem_arc]['name'])
			{
				...
				{										
						$sql_viejo = "UPDATE `documento_ambiental` SET `estado` = '2' 
					...
					$sql_guardar = "INSERT INTO `documento_ambiental` 
		...
		if($enviar_correo)
		{
			...
			WHERE usuario.estado = 1 AND componente.codigo= 'amb_ges' AND usuario.id_instancia= '".$_SESSION["nst"]."'
			LIMIT 1	";
			...
			if($row_correo["id"])
			{
				...
				El contratista actualizo datos de la documentaci&oacute;n ambiental, 
				...
				Sistema S.I.G.C ";
				 ...
				 enviar_correo(trim($row_correo["correo"]), ucwords(strtolower(trim($row_correo["nombre"]))), $mensaje, "Documentos AMBIENTAL actualizados en SIGC", NULL);
			}
		}
...
<h2>ACTUALIZAR DOCUMENTOS AMBIENTAL</h2>
<div align=right>
	<a href="?cmp=lista_amb_actualizar"> <i class="fa fa-reply fa-2x"></i> Volvel al listado de documentos </a>
</div>
...
while($row = mysql_fetch_array($res))
{	
	$sql_cond = "SELECT * FROM documento_ambiental WHERE id_empresa = '".$id_empresa."' AND id_documento = '".$row["id"]."' AND estado=1 LIMIT 1 ";
