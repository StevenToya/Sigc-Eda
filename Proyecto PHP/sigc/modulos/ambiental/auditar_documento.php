...
if($_POST["guardar"])
{
	$sql = "SELECT id FROM documento_ambiental WHERE documento_ambiental.id_empresa = '".$id_empresa."' AND documento_ambiental.estado='1' AND documento_ambiental.fase='1' ";
	$res = mysql_query($sql);
	while($row = mysql_fetch_array($res))
	{
		...
		{	
			$a++;
			$sql_act = "UPDATE `documento_ambiental` SET `fase` = '2', `observacion` = '".limpiar($_POST[$tem2])."' WHERE  id = '".$row["id"]."' ;";
			mysql_query($sql_act);
		}
		...
		{	
			$r++;
			$sql_act = "UPDATE `documento_ambiental` SET `fase` = '3', `observacion` = '".limpiar($_POST[$tem2])."'  WHERE  id = '".$row["id"]."' ;";
			mysql_query($sql_act);
		}
	}		
...
<h2>REVISI&Oacute;N DE DOCUMENTOS</h2>
<div align=right>
	<a href="?cmp=lista_amb_gestion"> <i class="fa fa-reply fa-2x"></i> Volvel al listado de documentos entregados </a>
</div><br>
...
<?php
$cont_td = 1;
$sql = "SELECT * FROM documento WHERE tipo = 5 AND id_instancia = '".$_SESSION["nst"]."' ";
$res = mysql_query($sql);
while($row = mysql_fetch_array($res))
{	
	$sql_cond = "SELECT * FROM documento_ambiental 
	WHERE id_empresa = '".$id_empresa."' AND id_documento = '".$row["id"]."' AND estado=1 LIMIT 1 ";
												...
												<tr>
													...
													<td valign=top  width="55%">
															...
															<?php if($row_cond["fase"]<>1){ ?>
																Estado: <?php echo $estado_doc; ?><br><br>
																<?php if($row_cond["observacion"]){ ?>																
																	<b>Tu observacion:</b>
																	<?php echo $row_cond["observacion"]; ?>
																<?php } ?>
															...
															<select name="gestion_<?php echo $row_cond["id"] ?>">
																...
																<br><br>		
															<b>Observacion:</b>
															<textarea cols=50 rows=3 name="obser_<?php echo $row_cond["id"] ?>"></textarea>
