...
?>
<h2>LISTA DE DOCUMENTOS PARA GESTIONAR AMBIENTAL</h2>
<div align=right> 						
	<a href='?cmp=editar_documento'><i class="fa fa-folder-open fa-2x"></i>Actualizar documentacion</a>&nbsp;&nbsp;&nbsp;&nbsp;
	<a href='?cmp=editar_basico'><i class="fa fa-pencil-square-o fa-2x"></i>Actualizar datos basicos</a>&nbsp;&nbsp;&nbsp;&nbsp;
</div>
	...
	<div class="panel-body">
		<div class="table-responsive">
			...
			<table class="table table-striped table-bordered table-hover" id="dataTables-example">
				...
				if(!$row["fecha_registro_ambiental"] || substr($row["fecha_registro_ambiental"],0 ,10) == '0000-00-00')
				{$estado = "<font color=red>Sin gestionar</font>";}
				else
				{
					if($row["estado_ambiental"]==1){	$estado = "<font color=#8A4B08>Sin revisar</font>";	}
				
					if($row["estado_ambiental"]==3)	{	$estado = '<font color=red>RECHAZADO</font>';	}
					
					if($row["estado_ambiental"]==2)	
					{
						$cant_dias = '356 days';
						$fecha = date_create(substr($row["fecha_registro_ambiental"],0 ,10));				
				...
				<tr class="odd gradeX">
						...
						<td align=center><?php echo substr($row["fecha_registro_ambiental"], 0, 10);  ?> </td>
						<td align=center><?php echo $row["fecha_registro_ambiental"] ?> </td>						
				...
				{
					$sql_emp = "SELECT * FROM documento_ambiental WHERE id_empresa = '".$id_empresa."' AND id_documento= '".$row["id"]."' AND estado = 1 LIMIT 1";
