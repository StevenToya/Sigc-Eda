<?php
	...
	if($_POST["pedido"])
	{
		$sql = "SELECT * FROM une_pedido WHERE une_pedido.numero = '".$_POST["pedido"]."' AND une_pedido.tipo='2' LIMIT 1	";
	}else{
		
		$sql = "SELECT * FROM une_pedido WHERE une_pedido.id = '".$_GET["id"]."' AND une_pedido.tipo='2' LIMIT 1	";
	}
	...
	if($_GET["conf"]==1)
	{
		...
		`une_pedido`.`id` = '".$_GET["id"]."' AND une_pedido.tipo='2' LIMIT 1; ";
		mysql_query($sql_upd);	
	}
...
<h2>DETALLES DEL PEDIDO <b><?php echo $row["numero"] ?></b></h2>

<div align=right>
	<a href="?cmp=lista_pendiente"> <i class="fa fa-reply fa-2x"></i> Volvel al listado pedidos </a>
</div>
