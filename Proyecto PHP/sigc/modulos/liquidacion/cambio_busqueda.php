...
if($_POST["guardar"])
{
	...
	if($_POST["item"]){$_SESSION["ss_item"] = $_POST["item"];}else{$_SESSION["ss_item"] = "";}
	...
	if($_POST["fecha_inicial"]){$_SESSION["ss_fecha_inicial"] = $_POST["fecha_inicial"];}else{$_SESSION["ss_fecha_inicial"] = date("Y-m-d");}
	if($_POST["fecha_final"]){$_SESSION["ss_fecha_final"] = $_POST["fecha_final"];}else{$_SESSION["ss_fecha_final"] = date("Y-m-d");}	
	
	 echo "<META HTTP-EQUIV='refresh' CONTENT='0; URL=?cmp=gestionar'>";
	 die();	
}

if(!$_SESSION["ss_fecha_final"]){ $_SESSION["ss_fecha_final"] = date("Y-m-d");}
if(!$_SESSION["ss_fecha_inicial"]){ $_SESSION["ss_fecha_inicial"] = date("Y-m-d");}

?>

<h2>MODIFICAR CONFIGURACION DE BUSQUEDA PARA LIQUIDACION </h2>
					 ...
					 <td valign=top  width='50%'>
						...
						<div class=" form-group input-group input-group-lg" style="width:100%">
						  <span class="input-group-addon">Item</span>
							<select name="item" class="form-control">
								<option value="">Todas</option>
								<?php
								$sql = "SELECT item  FROM `liquidacion_zona` WHERE item <> '' GROUP BY item ORDER BY item";
								$res = mysql_query($sql);
								while($row = mysql_fetch_array($res)){
								?>
									<option <?php if($row["item"]==$_SESSION["ss_item"]){ ?> selected <?php } ?>><?php echo $row["item"]; ?></option>
								<?php
								}
								?>
							</select>
						...
						<div class=" form-group input-group input-group-lg" style="width:100%">
						  <span class="input-group-addon">Fecha Inicial</span>
							<input type="text" id="fecha_inicial" name="fecha_inicial" value="<?php echo $_SESSION["ss_fecha_inicial"] ?>" class="form-control">
					...
					<td valign=top width='50%'>
						...
						<div class=" form-group input-group input-group-lg" style="width:100%">
						  <span class="input-group-addon">Servicio</span>
							<select name="servicio" class="form-control">
								<option value="">Todas</option>
								<?php
								$sql = "SELECT servicio  FROM `liquidacion_zona` WHERE servicio <> '' GROUP BY servicio ORDER BY servicio";
								$res = mysql_query($sql);
								while($row = mysql_fetch_array($res)){
								?>
									<option <?php if($row["servicio"]==$_SESSION["ss_servicio"]){ ?> selected <?php } ?>><?php echo $row["servicio"]; ?></option>
								<?php
								}
								?>
							</select>
						...
						<div class=" form-group input-group input-group-lg" style="width:100%">
						  <span class="input-group-addon">Fecha Final</span>
							<input type="text" id="fecha_final" name="fecha_final" value="<?php echo $_SESSION["ss_fecha_final"] ?>" class="form-control">
							<script type="text/javascript">
								var opts = {                            
