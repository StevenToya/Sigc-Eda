...
{
	$valor_tem = "che_valor_".$row["id"]; 
	$observacion_tem =  "obs_".$row["id"]; 
	if($_POST[$valor_tem])
	{
		$sql = "UPDATE `tramite` SET 
		`id_usuario_revisa` = '".$_SESSION["user_id"]."', 
		`contratista_valor` = '".$_POST[$valor_tem]."',  
		`observacion_contratista` = '".$_POST[$observacion_tem]."'
		...
		tecnologia.nombre AS nom_tecnologia, tipo_paquete, 	contratista_valor, contratista_equipo, contratista_material, observacion_contratista
			...
			while($row = mysql_fetch_array($res))
			{
				...
				<tr>
					...
					<td align=right><b> $<?php echo moneda($row["valor_liquidado"]); ?></b></td>
					...
					<td align=center>
						<?php echo $contratista_valor ?>
					  <!--  Equip:<?php echo $contratista_equipo ?><br>
						Mater:<?php echo $contratista_material ?><br> -->
					</td>
					...
					-->
					<td align=center><textarea name="obs_<?php echo $row["id"] ?>" ><?php echo $row["observacion_contratista"] ?></textarea> </td>
