...
$sql = " SELECT tramite.ot, tipo_trabajo.nombre AS nom_tt , tipo_trabajo.codigo, tecnologia.nombre, tipo_trabajo.tipo, tramite.id, valor_liquidado,
		...
		observacion_edatel, observacion_edatel_material, observacion_edatel_equipo
					...
					WHERE tramite.estado_liquidacion=2 AND tramite.contratista_equipo IN ('3','4') 
				 ";
$res = mysql_query($sql);
while($row = mysql_fetch_array($res))
{
	...
	if($_POST[$equipo_tem])
	{
		...
		`id_usuario_liquida_equipo` = '".$_SESSION["user_id"]."', 
		`contratista_equipo` ='".$_POST[$equipo_tem]."',  
		`observacion_edatel_equipo` = '".$_POST[$observacion_tem]."'
...
<h2>TRAMITES RECHAZADOS POR EQUIPOS</h2>
<div align=right>
	<a href="?cmp=panel_equipo"> <i class="fa fa-reply fa-2x"></i> Volver al panel inicial</a>
</div>
		...
		observacion_edatel, observacion_edatel_material, observacion_edatel_equipo, observacion_contratista_equipo
			...
			while($row = mysql_fetch_array($res))
			{
				...
				<tr>
					...
					<td align=center><?php echo $contratista_equipo ?></td>					
					<td><?php echo $row["observacion_contratista_equipo"] ?></td>					
					...
					<td align=center style="background-color:#E3F6CE">
						<?php if($row["contratista_equipo"]!=3){ ?>
							-----
						<?php }else{ ?>
					...
					<td align=center><textarea name="obs_<?php echo $row["id"] ?>" ><?php echo $row["observacion_edatel_equipo"]; ?></textarea> </td>
