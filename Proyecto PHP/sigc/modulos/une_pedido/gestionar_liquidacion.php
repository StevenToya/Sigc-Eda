...
<h2>LIQUIDACION SIN FINALIZAR <b>HFC</b></h2>


<table width=100%>
	<tr>
		<td>
			<form action="?cmp=gestionar_detalle_liquidacion"  method="post" name="form" id="form"  enctype="multipart/form-data">
				<div class="row">
				   <div class="col-lg-6">
	...
	<tr>
		<td>
			...
			$sql_tot = "SELECT COUNT(*) AS cantidad FROM une_pedido 
				WHERE fecha >= '".$ano_ges."-".$mm."-01' 
				AND fecha <= '".$ano_ges."-".$mm."-31' 
				AND  une_pedido.tipo='1' 
				AND une_pedido.estado_liquidacion IS NOT NULL 
				AND une_pedido.ciudad IN ('Monteria','Sincelejo','Valledupar')";
			$res_tot = mysql_query($sql_tot);
			$row_tot = mysql_fetch_array($res_tot);
			?>
			<a href="modulos/une_pedido/liquidacion.php?excel=1&mm=<?php echo $mm ?>&aa=<?php echo $ano_ges	 ?>" target=blank>
			...
			<?php
				...
				if($row_cont["estado_liquidacion"]=='1')$ees = "Pend. por EDATEL";
				if($row_cont["estado_liquidacion"]=='4')$ees = "Pend. por EIA";
				if($row_cont["estado_liquidacion"]=='3')$ees = "Rechazado";
				if($row_cont["estado_liquidacion"]=='2')$ees = "Confirmado";
				
			?>
				<a href="modulos/une_pedido/liquidacion.php?excel=1&mm=<?php echo $mm ?>&aa=<?php echo $ano_ges	?>&ee=<?php echo $row_cont["estado_liquidacion"] ?>" target=blank>
					<button class="btn btn-primary" type="button">
...
<div class="panel panel-default" style="width:100%;">
   <div align=left class="panel-heading">Pedidos sin confirmar liquidacion </div>
  <table class="table" align=center class="table table-striped table-bordered table-hover" id="dataTables-example">
   ...
   $sql_pedido = "SELECT * FROM une_pedido WHERE estado_liquidacion IN ('4')  
   AND fecha >= '".$ano_ges."-".$mm."-01' 
   AND fecha <= '".$ano_ges."-".$mm."-31' 
   AND une_pedido.tipo='1' 
   AND une_pedido.estado_liquidacion IS NOT NULL  
   AND une_pedido.ciudad IN ('Monteria','Sincelejo','Valledupar')";
   ...
   {		
			...
			if($row_pedido["estado_liquidacion"]==1){ $estado_liquidacion = "<font color=#000000>Pend. Edatel</font>"; }
	...
	?>
			<tr>
				...
				<td align=center><a href="?cmp=gestionar_detalle_liquidacion&id=<?php echo $row_pedido["id"] ?>"> <font <?php echo $font ?> ><i class="fa fa-eye fa-2x"> </i></font> </a></td></td>
		   </tr>
		   <?php 
