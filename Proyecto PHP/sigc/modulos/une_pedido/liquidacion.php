...
if($_GET["excel"])
{
	...
	?>
		<table border=1>
			<tr>
				...
				<th>departamento</th>
				<th>microzona</th>
				<th>strIdentificacion</th>				
				<th>strNombre</th>
				<th>strCodigoFuncionario</th>
				<th>strSegmento</th>
				<th>migTras</th>	
				<th>tipoInstaEspecifico</th>	
				<th>tipoInstaGeneral</th>	
				<th>grupoProducto</th>	
				<th>portafolio</th>	
				<th>lngCantidad</th>	
				<th>codActividad</th>	
				<th>tecnologia</th>	
				<th>actividadHomologada</th>	
	...
	WHERE ".$query ." fecha >= '".$_GET["aa"]."-".$_GET["mm"]."-01' 
	AND fecha <= '".$_GET["aa"]."-".$_GET["mm"]."-31' 
	...
	while($row = mysql_fetch_array($res))
	{
					...
					?>
						<tr>
							...
							<td><?php echo $row["ciudad"] ?></td>
							<td><?php echo $row["stridentificacion"] ?></td>
							<td><?php echo $row["nombre_funcionario"] ?></td>
							<td><?php echo $row["codigo_funcionario"] ?></td>
							<td><?php echo $row["segmento"] ?></td>
							<td><?php echo $row["migtras"] ?></td>
							<td><?php echo $row["tipoinstaespecifico"] ?></td>
							<td><?php echo $row["tipoinstageneral"] ?></td>
							<td><?php echo $row["producto_homologado"] ?></td>
							<td><?php echo $row["portafolio"] ?></td>
							<td>1</td>
							<td><!-- codActividad --></td>
							<td><?php echo $row["tecnologia"] ?></td>
							<td><?php echo $row_item["codigo"] ?><font color=red><?php echo $row_item["cc"] ?></font></td>
							<td><?php echo $row_item["valor"] ?></td>
							<td><?php echo $row_item["valor"] ?></td>						
						</tr>
...
<table width="100%">
	<tr>
		<td>
			...
			<?php
				$sql_cont = "SELECT COUNT(*) AS cantidad, estado_liquidacion  
				FROM une_pedido  
				WHERE 
					fecha >= '".$ano_ges."-".$mm."-01' AND fecha <= '".$ano_ges."-".$mm."-31' 
					AND une_pedido.tipo='1'  
					AND une_pedido.estado_liquidacion IS NOT NULL
					AND une_pedido.ciudad IN ('Monteria','Sincelejo','Valledupar')
...
<div class="panel panel-default" style="width:100%;">
  ...
  <thead>
   ...
   $sql_pedido = "SELECT * FROM une_pedido 
				...
				AND une_pedido.tipo='1' 
				AND une_pedido.ciudad IN ('Monteria','Sincelejo','Valledupar') ".$query." ;";
