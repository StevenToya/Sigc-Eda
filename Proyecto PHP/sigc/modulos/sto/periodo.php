...
if($_GET["at"])
{
	$sql = "UPDATE `sto_periodo` SET `estado` = '2' ;";
	mysql_query($sql);
	
	$sql = "UPDATE `sto_periodo` SET `estado` = '1' WHERE id = '".$_GET["at"]."' LIMIT 1;";
	mysql_query($sql);
}

if($_GET["eliminar"])
{
	$sql = "DELETE FROM `sto_periodo` WHERE `id` = '".$_GET["eliminar"]."' LIMIT 1;";
	mysql_query($sql);		
}
...
{	
	...
	$sql = "INSERT INTO `sto_periodo` (	fecha_inicial, fecha_final, estado, fecha_registro)
	...
	<div class="panel-body">
		<div class="table-responsive">
			...
			<table class="table table-striped table-bordered table-hover" >
				...
				<tbody>
				<?php
				$sql = "SELECT * FROM sto_periodo ORDER BY id DESC";
				$res = mysql_query($sql);
				...
				?>
					<tr class="odd gradeX">
						...
						<?php
						$sql_bb = "SELECT id FROM sto_ejecutado WHERE id_periodo = '".$row["id"]."' LIMIT 1 ";
