	...
	<tr>
		...
		<th>Valor  alimentacion  y  aloj. pernoctado</th>				
				...
				$sql = "SELECT sto_persona.nombre , sto_persona.zona , sto_persona.localidad , sto_persona.fecha_registro, sto_sap.cuenta AS cue_sap,
						...
						sto_persona.id, sto_persona.identificacion, municipio.nombre AS nom_municipio, usuario.nombre AS nom_coordinador, usuario.apellido AS ape_coordinador
			   FROM sto_persona 
				...
				WHERE  sto_persona_item.id_item=13
...
if($_GET["abrir"])
{ 
	$sql = "DELETE FROM `sto_periodo_usuario_cmv` WHERE `sto_periodo_usuario_cmv`.`id` = '".$_GET["abrir"]."' LIMIT 1 ";
	mysql_query($sql);
}
...
<?php
	...
	while($row = mysql_fetch_array($res))
	{
		...
		$sql_cont = " SELECT SUM(IF(tipo=1,1,0)) AS ad, SUM(IF(tipo=2,1,0)) AS ah ,
						SUM(IF(tipo=1,sto_ejecutado_cmv.valor,0)) AS ad_total, SUM(IF(tipo=2,sto_ejecutado_cmv.valor,0)) AS ah_total 
						FROM sto_ejecutado_cmv
					INNER JOIN 
							sto_persona ON sto_ejecutado_cmv.id_persona = sto_persona.id
					INNER JOIN sto_persona_item 
							ON sto_persona.id = sto_persona_item.id_persona
					WHERE
						id_usuario = '".$row["id_coordinador"]."'
						 AND sto_persona_item.id_item=13
						AND id_periodo = '".$row_per["id"]."'	
						";
		$res_cont = mysql_query($sql_cont);
		$row_cont = mysql_fetch_array($res_cont);
		...
		FROM sto_periodo_usuario_cmv
		WHERE sto_periodo_usuario_cmv.id_usuario = '".$row["id_coordinador"]."' AND sto_periodo_usuario_cmv.id_periodo = '".$row_per["id"]."' ";
