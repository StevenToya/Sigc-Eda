...
{	
	...
	$sql_bus = "SELECT estado, id FROM sto_persona WHERE identificacion = '".$_POST["identificacion"]."' LIMIT 1";
	...
	if($row_bus["id"])
	{
		if($row_bus["estado"]==2)
		{
			$sql = "UPDATE `sto_persona` SET `estado` = '1'	WHERE `id` = '".$row_bus["id"]."' LIMIT 1;";
			mysql_query($sql);	
	...
	else
	{
		...
		$sql = "INSERT INTO `sto_persona` (`id_usuario` ,`id_plataforma` , `id_sap` , `id_car` , `id_rubro` , 
		...
		if($id_persona)
		{		
			$sql_item = "INSERT INTO sto_persona_item (id_persona, id_item) VALUES ('".$id_persona."', '".$_POST["id_rol"]."')";
			...
			while($row_reco = mysql_fetch_array($res_reco))
			{
				...
				if($_POST[$tem])
				{
					$sql_item = "INSERT INTO sto_persona_item (id_persona, id_item) VALUES ('".$id_persona."', '".$row_reco["id"]."')";
					mysql_query($sql_item);
				}
			}
