...
$vector_rol[] = "Iniciar";
$sql_rol = "SELECT stohome_ejecutado.fecha 
FROM  `stohome_ejecutado`
INNER JOIN stohome_item ON stohome_ejecutado.id_item = stohome_item.id 
WHERE 
fecha >= '".$row_per["fecha_inicial"]."' 
AND stohome_ejecutado.fecha <= '".$row_per["fecha_final"]."' 
AND stohome_ejecutado.id_periodo = '".$row_per["id"]."'
AND stohome_item.tipo=1
...
{	
	...
	$sql_del = "DELETE FROM `stohome_ejecutado_cmv` WHERE `id_periodo` = '".$row_per["id"]."'  AND id_persona = '".$_GET["idp"]."'  ";
	mysql_query($sql_del);	
	...
	foreach ($vector AS $k => &$valor) 
	{
		...
		{
				$valor_ad =   12393.60;
			 $sql_g = "INSERT INTO `stohome_ejecutado_cmv` 
...
<div align=right>
	<a href="?cmp=lista_personal_cmv&id=<?php echo $_GET["id"] ?>"> <i class="fa fa-reply fa-2x"></i> Volvel al listado de coordinadores </a>
</div> 
						...
						<tr bgcolor="silver">
							...
							{			
								...
								else
								{
									...
									$sql_eje = "SELECT id, tipo FROM stohome_ejecutado_cmv WHERE 
									...
									<td <?php echo $color_cel ?> valign=top> 
										...
										<div align=left>		
													<input  <?php echo $checked1; ?> disabled style="width:15px;height:15px;" type='checkbox' name='ch_<?php echo $fecha_comp ?>_ad' id="ch_<?php echo $campo ?>_ad">Ali.Dia<br>
													<input  <?php echo $checked2; ?> disabled style="width:15px;height:15px;" type='checkbox' name='ch_<?php echo $fecha_comp ?>_ah' id="ch_<?php echo $campo ?>_ah">Ali+Hot.
										</div>		
										<?php
										}else{  							
										?>
