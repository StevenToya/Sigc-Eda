...
{	return number_format($dato, 2, ',', '.'); }
	...
	$i = 0;
	$sql = "SELECT stohome_item.id, stohome_item.nombre, stohome_item.valor 
...
<table border=1>				
	<tr>
		...
		<th bgcolor=red><font color='#FFFFFF'>OBSERVACION - NOVEDADES </font></th>
				...
				<?php
				$vec_tem = $vec_item; 
				$sql = "SELECT stohome_persona.nombre , stohome_persona.zona ,  stohome_persona.identificacion , stohome_persona.localidad , stohome_persona.fecha_registro, stohome_sap.cuenta AS cue_sap,
						...
						stohome_persona.identificacion, municipio.nombre AS nom_municipio, stohome_persona.id AS idp, usuario.nombre AS usu_nombre, usuario.apellido AS usu_apellido						
			   FROM stohome_persona 
				...
				?>
					<tr class="odd gradeX">
						...
						?>
							<td><?php
										$sql_nov = "SELECT * FROM  `stohome_no_ejecutado` 
...
if($_GET["abrir"])
{
	$sql = "DELETE FROM `stohome_periodo_usuario` WHERE `id_usuario` = '".$_GET["abrir"]."' AND id_periodo = '".$row_per["id"]."' ";
	mysql_query($sql);
}
...
<?php
	...
	while($row = mysql_fetch_array($res))
	{
		...
		FROM stohome_ejecutado
		INNER JOIN stohome_persona ON stohome_ejecutado.id_persona = stohome_persona.id
		WHERE stohome_persona.id_usuario = '".$row["id_coordinador"]."' AND stohome_ejecutado.id_periodo = '".$row_per["id"]."' ";
		$res_tot = mysql_query($sql_tot);
		$row_tot = mysql_fetch_array($res_tot);
		
		$sql_cierre = "SELECT fecha_registro, id 
		FROM stohome_periodo_usuario
		WHERE stohome_periodo_usuario.id_usuario = '".$row["id_coordinador"]."' AND stohome_periodo_usuario.id_periodo = '".$row_per["id"]."' ";
