...
{		
	...
	if(!mysql_error())
	{		
		$sql_del = "DELETE FROM `stohome_persona_item` WHERE `id_persona` = '".$_GET["id"]."' ";
		mysql_query($sql_del);	
		
		
		$sql_item = "INSERT INTO stohome_persona_item (id_persona, id_item) VALUES ('".$_GET["id"]."', '".$_POST["id_rol"]."')";
		...
		while($row_reco = mysql_fetch_array($res_reco))
		{
			...
			if($_POST[$tem])
			{
				$sql_item = "INSERT INTO stohome_persona_item (id_persona, id_item) VALUES ('".$_GET["id"]."', '".$row_reco["id"]."')";
				mysql_query($sql_item);
			}
		}
 ...
 $sql = "SELECT stohome_persona.nombre , stohome_persona.zona , stohome_persona.localidad , stohome_persona.fecha_registro, stohome_sap.id AS sap_id, stohome_car.id AS car_id,
				   ...
				   FROM stohome_persona 
					...
					WHERE  stohome_item.tipo = '1' AND stohome_persona.id = '".$_GET["id"]."'  LIMIT 1 ";
$res = mysql_query($sql);
	...
	<div class="panel panel-primary">
		...
		<div class="panel-body" align=center>
			<table border=0 width=80%>				
				...
				{
					$sql_item = "SELECT id FROM stohome_persona_item  WHERE id_persona ='".$_GET["id"]."' AND id_item ='".$row["id"]."' LIMIT 1";
