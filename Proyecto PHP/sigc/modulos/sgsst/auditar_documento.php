...
if($_POST["guardar"])
{
	$sql = "SELECT id FROM documento_empresa WHERE documento_empresa.id_empresa = '".$id_empresa."' AND documento_empresa.estado='1' AND documento_empresa.fase='1' ";
	$res = mysql_query($sql);
	while($row = mysql_fetch_array($res))
	{
		...
		{	
			$a++;
			$sql_act = "UPDATE `documento_empresa` SET `fase` = '2', `observacion` = '".limpiar($_POST[$tem2])."' WHERE  id = '".$row["id"]."' ;";
			mysql_query($sql_act);
		}
		...
		{	
			$r++;
			$sql_act = "UPDATE `documento_empresa` SET `fase` = '3', `observacion` = '".limpiar($_POST[$tem2])."'  WHERE  id = '".$row["id"]."' ;";
			mysql_query($sql_act);
		}
	}		
...
<div align=right>
	<a href="?cmp=lista_sst_gestion"> <i class="fa fa-reply fa-2x"></i> Volvel al listado de documentos entregados </a>
</div><br>
...
while($row = mysql_fetch_array($res))
{	
	$sql_cond = "SELECT * FROM documento_empresa 
